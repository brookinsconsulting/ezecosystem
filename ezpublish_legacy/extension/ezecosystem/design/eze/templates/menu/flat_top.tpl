<div class="topmenu-design">
    ...
    {def $root_node=fetch( 'content', 'node', hash( 'node_id', $pagedata.root_node) )
         $top_menu_items=fetch( 'content', 'list', hash( 'parent_node_id', $root_node.node_id,
                                                          ...
                                                          'class_filter_array', ezini( 'MenuContentSettings', 'TopIdentifierList', 'menu.ini' )|append( 'article') ) )
...
{* <h2>{$item.data_map.exclude_from_top_menu.content} - {$item.data_map.exclude_from_top_menu|attribute(show,1)}</h2><hr> *}
            {* if array( 'Community', 'Planetarium', 'Blogs', 'About', 'About-eZ-Publish', 'Contact-Us' )| contains( $item.url_alias )|not *}

            {if $item.data_map.exclude_from_top_menu.content|eq( 0 )}
