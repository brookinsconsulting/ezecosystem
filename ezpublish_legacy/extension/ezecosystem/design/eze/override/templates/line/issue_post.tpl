...
<div class="content-view-line">
    ...
    *}
	...
	<span>{$node.data_map.publication_date.content.timestamp|l10n(shortdatetime)}</span> &nbsp; <a href={$node.parent.data_map.blog.content|ezurl()}>{$node.parent.data_map.blog.data_text}</a> &nbsp; <span class="mirror"><a href="{$node.url_alias|ezurl(no)}">Mirror</a></span> &nbsp; <span class="link"><a href="{$node.data_map.blog_post_url.content}">Link</a></span> {if and( $view_count_enabled, $node.view_count|gt( $view_count_threshold ) )}<span class="views"><a href="#" style="text-decoration:none;" title="View count @ {$node.view_count}">@{$node.view_count}</a></span>{/if}
    </div>
