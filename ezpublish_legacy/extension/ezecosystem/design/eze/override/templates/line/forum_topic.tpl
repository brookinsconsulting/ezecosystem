...
<div class="content-view-line">
    ...
    *}
    ...
    <div class="attribute-url">
        {if $sources_list_share_ez_no_forum_node_ids|contains( $node.parent.node_id )}
	<span>{$node.data_map.publication_date.content.timestamp|l10n(shortdatetime)}</span> &nbsp; <a href={$node.parent.url|ezurl()}>{$node.parent.parent.name} : {$node.parent.name}</a> &nbsp; <span class="mirror"><a href="{$node.url_alias|ezurl(no)}">Mirror</a></span> &nbsp; <span class="link"><a href="{$node.data_map.link.content}">Link</a></span> {if and( $view_count_enabled, $node.view_count|gt( $view_count_threshold ) )}<span class="views"><a href="#" style="text-decoration:none;" title="View count @ {$node.view_count}">@{$node.view_count}</a></span>{/if}
        {else}
