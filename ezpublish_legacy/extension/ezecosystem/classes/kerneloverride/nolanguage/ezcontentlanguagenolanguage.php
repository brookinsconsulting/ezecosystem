<?php
/**
 * File containing the eZContentLanguageNoLanguage class.
 *
...
class eZContentLanguageNoLanguage extends eZPersistentObject
{
    const MAX_COUNT = 30;
     ...
     */
    function eZContentLanguageNoLanguage( $row = array() )
    {
    ...
    static function definition()
    {
        static $definition = array( 'fields' => array( 'id' => array( 'name' => 'ID',
                      ...
                      'sort' => array( 'name' => 'asc' ),
                      'class_name' => 'eZContentLanguageNoLanguage',
     ...
     *             will be used.
     * \return eZContentLanguageNoLanguage object of the added language (or the existing one if specified language has been already used)
     *         or false in case of any error (invalid locale code or already reached eZContentLanguageNoLanguage::MAX_COUNT languages).
     * \static
     */
    static function addLanguage( $locale, $name = null )
    {
        ...
        $languages = eZContentLanguageNoLanguage::fetchList( true );

        if ( ( $existingLanguage = eZContentLanguageNoLanguage::fetchByLocale( $locale ) ) )
        {
            eZDebug::writeWarning( "Language '$locale' already exists!", __METHOD__ );
            return $existingLanguage;
        }

        if ( count( $languages ) >= eZContentLanguageNoLanguage::MAX_COUNT )
        {
        ...
        $newLanguage = new eZContentLanguageNoLanguage( array(
                                                  'id' => $candidateId,
                                                  'locale' => $locale,
                                                  'name' => $name,
                                                  'disabled' => 0 ) );
        ...
        eZContentLanguageNoLanguage::fetchList( true );
    ...
    {
        $language = eZContentLanguageNoLanguage::fetch( $id );
        if ( $language )
        {
     ...
     * \return Array of the eZContentLanguageNoLanguage objects of languages used on the site.
     * \static
     */
    static function fetchList( $forceReloading = false )
    {
        ...
        {
            $languages = eZPersistentObject::fetchObjectList( eZContentLanguageNoLanguage::definition() );
    ...
    {
        $languages = array();
        foreach ( eZContentLanguageNoLanguage::fetchList( $forceReloading ) as $language )
        {
   ...
   /**
    ...
    {
        $languages = eZContentLanguageNoLanguage::fetchList();
        $localeList = array();
     ...
     * \return eZContentLanguageNoLanguage object of language identified by ID $id.
     * \static
     */
     ...
     * \return eZContentLanguageNoLanguage object identified by locale code $locale.
     */
    static function fetchByLocale( $locale, $createIfNotExist = false )
    {
        ...
        if ( $createIfNotExist )
        {
            $language = eZContentLanguageNoLanguage::addLanguage( $locale );
        }
     ...
     *                     See eZContentLanguageNoLanguage::setPrioritizedLanguages().
     * \return Array of the eZContentLanguageNoLanguage objects of the prioritized languages.
     * \static
     */
    static function prioritizedLanguages( $languageList = false )
    {
        if ( !isset( $GLOBALS['eZContentLanguagePrioritizedLanguages'] ) )
        {
            ...
            foreach ( $languageList as $localeCode )
            {
                ...
                $language = eZContentLanguageNoLanguage::fetchByLocale( $localeCode );
                if ( $language )
                {
            ...
            {
                $completeList = eZContentLanguageNoLanguage::fetchList();
     ...
     * \see eZContentLanguageNoLanguage::prioritizedLanguages()
     * \static
     */
    static function prioritizedLanguageCodes()
    {
        $languages = eZContentLanguageNoLanguage::prioritizedLanguages();
        $localeList = array();
    ...
    static function setPrioritizedLanguages( $languages )
    {
        ...
        eZContentLanguageNoLanguage::prioritizedLanguages( $languages );
    }

    /**
     * Clears the prioritized language list set by eZContentLanguageNoLanguage::setPrioritizedLanguages and reloading
    ...
    {
        eZContentLanguageNoLanguage::setPrioritizedLanguages( false );
    }

    /**
     * Returns the most prioritized language.
     *
     * \return eZContentLanguageNoLanguage object for the most prioritized language.
     * \static
     */
    static function topPriorityLanguage()
    {
        $prioritizedLanguages = eZContentLanguageNoLanguage::prioritizedLanguages();
     ...
     * \return Array of eZContentLanguageNoLanguage objects of languages which have set the corresponding bit in $mask.
     */
     ...
     *
     * \param mask Bitmap specifying which languages should be returned.
     * \return Array of eZContentLanguageNoLanguage objects of prioritized languages which have set the corresponding bit in $mask.
     */
     ...
     * \return eZContentLanguageNoLanguage object of the most prioritized language which have set the corresponding bit in $mask.
     */
     ...
     *
     * \param languageLocaleList List of language locales to choose from.
     * \return eZContentLanguageNoLanguage object of the most prioritized language.
     */
    ...
    static function maskByLocale( $locales, $setZerothBit = false )
    {
        ...
        {
            $language = eZContentLanguageNoLanguage::fetchByLocale( $locale );
            if ( $language )
            {
                $mask |= $language->attribute( 'id' );
            }
    ...
    {
        $maxNumberOfLanguges = eZContentLanguageNoLanguage::MAX_COUNT;
        ...
        if ( $langMask < 0 or $langMask > $maxInteger or $langMask == 1 )
        {
            ...
            $defaultLanguage = eZContentLanguageNoLanguage::topPriorityLanguage();
        ...
        for ( $i = 1; $i < $maxNumberOfLanguges; ++$i )
        {
            ...
            if ( ($newMask & $mask) > 0 )
            {
                ...
                {
                    $list[] = eZContentLanguageNoLanguage::fetch( $newMask )->attribute( 'locale' );
                }
                else
                {
                    $list[] = $newMask;
    ...
    static function sqlFilter( $languageTable, $languageListTable = null, $languageAttributeName = 'language_id', $languageListAttributeName = 'language_mask', $lang = false )
    {
       ...
       $db = eZDB::instance();
        ...
        $prioritizedLanguages = eZContentLanguageNoLanguage::prioritizedLanguages();
        if ( is_string( $lang ) )
            $lang = eZContentLanguageNoLanguage::fetchByLocale( $lang );
        if ( $lang instanceof eZContentLanguageNoLanguage )
     ...
     *
     * \see eZContentLanguageNoLanguage::setCronjobMode()
     */
    static function clearCronjobMode()
    {
        eZContentLanguageNoLanguage::setCronjobMode( false );
    }
    ...
    {
        $localList = array();
        $languages = eZContentLanguageNoLanguage::prioritizedLanguagesByMask( $mask );
        foreach ( $languages as $key => $language )
        {
