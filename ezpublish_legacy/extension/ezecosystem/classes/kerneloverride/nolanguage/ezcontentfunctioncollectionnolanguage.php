<?php
/**
 * File containing the eZContentFunctionCollectionNoLanguage class.
 *
...
/*!
  \class eZContentFunctionCollectionNoLanguage ezcontentfunctioncollection.php
  \brief The class eZContentFunctionCollectionNoLanguage does

*/

class eZContentFunctionCollectionNoLanguage
{
    /*!
     Constructor
    */
    function eZContentFunctionCollectionNoLanguage()
    {
    }
    ...
    static public function fetchContentNode( $nodeID, $nodePath, $languageCode, $remoteID = false )
    {
        ...
        if ( $nodeID )
        {
            ...
            $contentNode = eZContentObjectTreeNodeNoLanguage::fetch( $nodeID, $languageCode );
        }
        else if ( $nodePath )
        {
            ...
            if ( $nodeID )
            {
               $contentNode = eZContentObjectTreeNodeNoLanguage::fetch( $nodeID );
            }
        }
        else if ( $remoteID )
        {
            $contentNode = eZContentObjectTreeNodeNoLanguage::fetchByRemoteID( $remoteID );
        }
        if ( $contentNode === null )
        {
                               ...
                               $groupBy, $mainNodeOnly, $ignoreVisibility, $limitation )
    {
        ...
        {
            $children = eZContentObjectTreeNodeNoLanguage::calendar( $treeParameters,
                                                            $parentNodeID );
        }
                              ...
                              $groupBy, $mainNodeOnly, $ignoreVisibility, $limitation, $asObject, $objectNameFilter, $loadDataMap = null )
    {
        ...
        {
            $children = eZContentObjectTreeNodeNoLanguage::subTreeByNodeID( $treeParameters,
                                                                  $parentNodeID );
        }
                                   ...
                                   $ignoreVisibility, $limitation, $mainNodeOnly, $extendedAttributeFilter, $objectNameFilter )
    {
        ...
        if ( is_numeric( $parentNodeID ) or is_array( $parentNodeID ) )
        {
            $childrenCount = eZContentObjectTreeNodeNoLanguage::subTreeCountByNodeID( array( 'Limitation' => $limitation,
                                                                           'ClassFilterType' => $class_filter_type,
                                                                           'ClassFilterArray' => $class_filter_array,
    ...
    static public function canInstantiateClassList( $groupID, $parentNode, $filterType = 'include', $fetchID, $asObject, $groupByClassGroup = false )
    {
        ...
        if ( is_numeric( $parentNode ) )
            $parentNode = eZContentObjectTreeNodeNoLanguage::fetch( $parentNode );
    ...
    static public function fetchTipafriendTopList( $offset, $limit, $start_time, $end_time, $duration, $ascending, $extended )
    {
        ...
        if ( $extended )
        {
            ...
            {
                $contentNode = eZContentObjectTreeNodeNoLanguage::fetch( $topList[ $key ][ 'node_id' ] );
        ...
        else
        {
            ...
            {
                $contentNode = eZContentObjectTreeNodeNoLanguage::fetch( $entry[ 'node_id' ] );
                if ( !is_object( $contentNode ) )
                    return array( 'error' => array( 'error_type' => 'kernel',
                                                    'error_code' => eZError::KERNEL_NOT_FOUND ) );
                $retList[] = $contentNode;
    ...
    {
        if ( $contentObject instanceof eZContentObjectTreeNodeNoLanguage )
        {
    ...
    static public function fetchRelatedObjectsID( $objectID, $attributeID, $allRelations)
    {
        ...
        foreach ( $allRelations as $relationType )
        {
            $relatedObjectsTyped[$relationType] =
                eZContentFunctionCollectionNoLanguage::fetchRelatedObjects( $objectID, $attributeID, array( $relationType ), false, array() );
        }
    ...
    static public function fetchReverseRelatedObjectsID( $objectID, $attributeID, $allRelations )
    {
        ...
        {
            $relatedObjectsTyped[$relationType] =
                eZContentFunctionCollectionNoLanguage::fetchReverseRelatedObjects( $objectID, $attributeID, array( $relationType ), false, array(), null );
        }
